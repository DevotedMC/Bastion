...
public class BastionSettingManager {
	...
	private void initSettings() {
		...
		bsiOverlay = new BooleanSetting(Bastion.getInstance(), true, "Display Bastion Information", "bsiOverlay", "Shows if the block you're standing on is bastioned territory.");
		showNoBastion = new BooleanSetting(Bastion.getInstance(), false, "Display if you are not in a bastion field", "showNoBastion", "If enabled, will display Bastion status, even if you are not currently in a bastion field");
		bsiLocation = new DisplayLocationSetting(Bastion.getInstance(), DisplayLocationSetting.DisplayLocation.SIDEBAR, "BSI Location", "bsiLocation", new ItemStack(Material.ARROW), "BSI");
		ignorePlacementWarnings = new BooleanSetting(Bastion.getInstance(), false, "Ignore placement warnings", "ignorePlacementWarnings", "Show placements warning in chat when placing in a bastion field");
